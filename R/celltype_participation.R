...
#' )
#' fun <- get("summarize_celltype_participation")
...
#' Print method for LogicCommParticipation
#' @param x LogicCommParticipation object.
#' @param ... Ignored.
#' @return Invisibly returns \code{x}, called for side effects.
#' @examples
#' participation <- structure(
#'   list(group_participation = data.frame(
#'     cell_type = "T", n_cells = 2, frac_communicating = 1,
#'     frac_sender_active = 0.5, frac_receiver_active = 0.5,
#'     total_active_lr_events = 1, importance_label = "high",
#'     is_major_hub = TRUE
#'   )),
#'   class = "LogicCommParticipation"
#' )
#' print(participation)
#' @export
...
#'   path = tempfile(fileext = ".csv")
#' )
#' fun <- get("plot_celltype_participation")
#' toy_args <- toy_args[intersect(names(toy_args), names(formals(fun)))]
#' try(do.call(fun, toy_args), silent = TRUE)
#' @export
...
#'   path = tempfile(fileext = ".csv")
#' )
#' fun <- get("plot_celltype_pathway_composition")
#' toy_args <- toy_args[intersect(names(toy_args), names(formals(fun)))]
#' try(do.call(fun, toy_args), silent = TRUE)
#' @export
...
#'   path = tempfile(fileext = ".csv")
#' )
#' fun <- get("plot_celltype_communication_profile")
#' toy_args <- toy_args[intersect(names(toy_args), names(formals(fun)))]
#' try(do.call(fun, toy_args), silent = TRUE)
#' @export
