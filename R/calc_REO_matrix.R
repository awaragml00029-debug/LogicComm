...
calc_REO_matrix <- function(expr_mat,
  ...
  if (is_bpcells) {
    ...
    return(.calc_REO_bpcells(expr_mat, lr_genes, rank_threshold, chunk_size,
                             return_rank, rank_output, anchor_genes, verbose))
  }
  ...
  if (verbose) message(sprintf(
    "[REO] Computing REO for %d retained genes x %d cells (anchors from %d genes; %.0f%% quantile).",
  ...
  for (ch in seq_len(n_chunks)) {
    ...
    result_cols[[ch]] <- fast_logic_scan(expr_chunk, anchors)
...
.calc_REO_bpcells <- function(bp_mat, lr_genes, rank_threshold, chunk_size,
                              ...
                              anchor_genes = NULL, verbose = TRUE) {
  ...
  n_chunks <- ceiling(n_cells / chunk_size)
  if (verbose) message(sprintf(
    "[REO/BPCells] Streaming %d chunks for %d cells; retaining %d genes; anchors from %d genes.",
