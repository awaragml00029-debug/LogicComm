...
summarize_communication_findings <- function(ct_comm, top_n = 10) {
  ...
  pair <- ct_comm$pair_summary[order(ct_comm$pair_summary$sum_lcs, decreasing = TRUE), , drop = FALSE]
  ...
  guide <- data.frame(
    ...
    recommended_metric = c("sum_lcs and n_active_lr", "lcs and n_active_edges", "role scores, role_separation_label, communication_evidence_label, and role_reliability_label", "n_edges and bootstrap/permutation diagnostics"),
    stringsAsFactors = FALSE
  )
  out <- list(
...
write_communication_report <- function(ct_comm,
  ...
  lines <- c(
    ...
    .df_to_md(findings$top_celltype_pairs[, intersect(c("sender_type", "receiver_type", "n_edges", "n_active_lr", "sum_lcs", "mean_lcs_active", "top_pathway"), names(findings$top_celltype_pairs)), drop = FALSE]),
    "",
